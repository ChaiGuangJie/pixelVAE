...
class ImEncoder(nn.Module):
    ...
    def __init__(self, in_size, zsize=2048, use_res=False, use_bn=False, depth=0, colors=3):
        a, b, c = 8, 16, 32  # 16, 32, 64 #16, 32, 64  # channel sizes
...
class ImDecoder(nn.Module):
    ...
    def __init__(self, in_size, zsize=32, use_res=False, use_bn=False, depth=0, out_channels=60):
        ...
        modules.extend([
            nn.Upsample(scale_factor=r, mode=upmode, align_corners=True),
            util.Block(c, c, deconv=True, use_res=use_res, batch_norm=use_bn),
            nn.Upsample(scale_factor=q, mode=upmode, align_corners=True),
            util.Block(c, b, deconv=True, use_res=use_res, batch_norm=use_bn),
            nn.Upsample(scale_factor=p, mode=upmode, align_corners=True),
