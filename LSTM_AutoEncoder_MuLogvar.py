...
ENCODER_INPUT_SIZE = 64  # rnn input size #/ image width
...
for epoch in range(EPOCH):
    for step, _data in enumerate(data_loader):  # gives batch data
        ...
        mu = data[:, 0, :, :]
        logvar = data[:, 1, :, :]
        output = rnn(torch.cat((mu, logvar), 2))  # rnn output input shape = torch.Size(c[64, 20, 32]) data[:, 0, :, :]
        kld_loss = KLDLossSeq(output[:, :, :Z_SIZE], output[:, :, Z_SIZE:])
        # cross entropy loss output[:, :, MU_SIZE:]
        mse_loss = MSELossSeq(output, torch.cat((mu, logvar), 2))  # BCE_loss(output, data)
        ...
        if (step + 1) % 30 == 0:  # todo test loader
            ...
            origin_z = model.sampler([mu, logvar])
            # # .reparameterize(data[:n, 0, :1024], data[:n, 0, 1024:])
            origin = model.decoder(origin_z[0, ::2, :, np.newaxis, np.newaxis])

            rec_z = model.sampler([output[0, ::2, :Z_SIZE], output[0, ::2, Z_SIZE:]])
