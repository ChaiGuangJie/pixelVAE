...
for epoch in range(EPOCH):
    for step, _data in enumerate(data_loader):  # gives batch data
        ...
        output = mu_rnn(data[:, 1, :, :])
        # mu: data[:, 0, :, :]  logvar: data[:, 1, :, :] rnn output input shape = torch.Size([64, 20, 32])
        kld_loss = KLDLossSeq(data[:, 0, :, :], output)  # cross entropy loss output[:, :, MU_SIZE:]
        mse_loss = MSELossSeq(output, data[:, 1, :, :])  # BCE_loss(output, data)
